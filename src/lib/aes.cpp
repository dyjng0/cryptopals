...
constexpr std::array<uint8_t, 256> INV_SBOX = {
    ...
    0x55, 0x21, 0x0c, 0x7d};
constexpr std::array<uint8_t, 256> GF_MULT_02 = {
    ...
    0xe3, 0xe1, 0xe7, 0xe5};
constexpr std::array<uint8_t, 256> GF_MULT_03 = {
    ...
    0x1f, 0x1c, 0x19, 0x1a};
constexpr std::array<uint8_t, 256> GF_MULT_09 = {
    ...
    0x5d, 0x54, 0x4f, 0x46};
constexpr std::array<uint8_t, 256> GF_MULT_11 = {
    ...
    0xbe, 0xb5, 0xa8, 0xa3};
constexpr std::array<uint8_t, 256> GF_MULT_13 = {
    ...
    0x80, 0x8d, 0x9a, 0x97};
constexpr std::array<uint8_t, 256> GF_MULT_14 = {
...
// helper functions
static void addRoundKey(std::span<uint8_t, BLOCK_SIZE> block,
...
static void invMixColumns(std::span<uint8_t, BLOCK_SIZE> block) {
  uint8_t s0, s1, s2, s3;
  for (size_t i = 0; i < BLOCK_SIZE; i += 4) {
    s0 = block[i];
    s1 = block[i + 1];
    ...
    s3 = block[i + 3];
    block[i] =
        GF_MULT_14[s0] ^ GF_MULT_11[s1] ^ GF_MULT_13[s2] ^ GF_MULT_09[s3];
    block[i + 1] =
        GF_MULT_09[s0] ^ GF_MULT_14[s1] ^ GF_MULT_11[s2] ^ GF_MULT_13[s3];
    block[i + 2] =
        GF_MULT_13[s0] ^ GF_MULT_09[s1] ^ GF_MULT_14[s2] ^ GF_MULT_11[s3];
    block[i + 3] =
        GF_MULT_11[s0] ^ GF_MULT_13[s1] ^ GF_MULT_09[s2] ^ GF_MULT_14[s3];
  }
}

static std::array<std::array<uint8_t, BLOCK_SIZE>, ROUNDS + 1>
