...
static constexpr int KEY_SCHED_SIZE = 176;
...
static constexpr uint8_t SBOX[256] = {
    ...
    0xb0, 0x54, 0xbb, 0x16};
static constexpr uint8_t RCON[10] = {0x01, 0x02, 0x04, 0x08, 0x10,
                                     0x20, 0x40, 0x80, 0x1b, 0x36};

// key schedule
std::array<uint8_t, KEY_SCHED_SIZE>
keyExpansion(std::span<const uint8_t, KEY_SIZE> key) {
  std::array<uint8_t, KEY_SCHED_SIZE> roundKeys;
  std::copy(key.begin(), key.end(), roundKeys.begin());

  for (size_t i = 4; i < KEY_SCHED_SIZE / WORD_SIZE; ++i) {
